...
export function ShareScreen() {
  ...
  async function saveAsImage() {
    ...
    try {
      ...
      if ((error as { name?: string }).name !== 'AbortError') setSaveFailed(true)
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <main className="mx-auto max-w-page px-6 py-10">
      ...
      <div ref={captureRef} className="mt-4 rounded-card bg-canvas p-2 sm:p-4">
      ...
      {saveFailed ? (
        <p className="mt-4 text-body text-ember">Не вдалося створити картинку</p>
      ) : null}
